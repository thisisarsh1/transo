...
marathi_max_len = max(len(seq) for seq in marathi_seq)

# Pad sequences
english_seq = tf.keras.preprocessing.sequence.pad_sequences(english_seq, maxlen=english_max_len, padding='post')
marathi_seq = tf.keras.preprocessing.sequence.pad_sequences(marathi_seq, maxlen=marathi_max_len, padding='post')
...
decoder_inputs = Input(shape=(marathi_max_len,))
...
target_data = marathi_seq[:, 1:]  # Adjust the target data to exclude the first element
model.fit([english_seq, marathi_seq[:, :-1]], np.expand_dims(target_data, -1), epochs=50, batch_size=1)
...
def translate(input_sentence):
    ...
    prediction = model.predict([input_sequence, np.zeros((1, marathi_max_len-1))])
